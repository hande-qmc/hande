...
contains
    ...
    subroutine init_momentum_symmetry(sys)
        ...
        use basis, only: basis_global, write_basis_fn
        use system
        ...
        sys%sym0 = 1
        sys%nsym = basis_global%nbasis/2 ! two spin orbitals per wavevector
        ...
        case(hub_k)
            ...
            gamma_sym = 0
            do i = 1, sys%nsym
                if (all(basis_global%basis_fns(i*2)%l == 0)) gamma_sym = i
            end do
            if (gamma_sym == 0) call stop_all('init_momentum_symmetry', 'Gamma-point symmetry not found.')

            do i = 1, sys%nsym
                do j = i, sys%nsym
                    ksum = basis_global%basis_fns(i*2)%l + basis_global%basis_fns(j*2)%l
                    do k = 1, sys%nsym
                        if (is_reciprocal_lattice_vector(sys, ksum - basis_global%basis_fns(k*2)%l)) then
                            sym_table(i,j) = k
                            sym_table(j,i) = k
                            if (k == gamma_sym) then
                                inv_sym(i) = j
            ...
            if (parent) then
                ...
                do i = 1, sys%nsym
                    ...
                    call write_basis_fn(sys, basis_global%basis_fns(2*i), new_line=.false., print_full=.false.)
        ...
        case(ueg)
            ...
            sys%nsym = basis_global%nbasis/2
    ...
    elemental function cross_product_ueg(sys, s1, s2) result(prod)
        ...
        k(:sys%lattice%ndim) = basis_global%basis_fns(2*s1)%l + basis_global%basis_fns(2*s2)%l
    ...
    pure function symmetry_orb_list_ueg(sys, orb_list) result(isym)
        ...
        do i = lbound(orb_list, dim=1), ubound(orb_list, dim=1)
            ...
            k = k + basis_global%basis_fns(orb_list(i))%l
        end do
