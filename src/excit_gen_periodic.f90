...
private
public :: gen_excit_periodic_complex, gen_excit_periodic_complex_no_renorm

contains

!--- Excitation generation ---

    subroutine gen_excit_periodic_complex(rng, sys, excit_gen_data, cdet, pgen, connection, hmatel, allowed_excitation)
        ...
        if (get_rand_close_open(rng) < excit_gen_data%pattempt_single) then
            ...
            call choose_ia_periodic(rng, sys, int(sys%read_in%mom_sym%gamma_sym), cdet%f, cdet%occ_list, cdet%symunocc, &
                               connection%from_orb(1), connection%to_orb(1), allowed_excitation)
            connection%nexcit = 1

            if (allowed_excitation) then
                ! 3a. Probability of generating this excitation.
                pgen = excit_gen_data%pattempt_single*calc_pgen_single_periodic(sys, int(sys%read_in%mom_sym%gamma_sym), &
                                cdet%occ_list, cdet%symunocc, connection%to_orb(1))
            ...
            call choose_ab_periodic(rng, sys, cdet%f, ij_sym, ij_spin, cdet%symunocc, connection%to_orb(1), &
            ...
            if (allowed_excitation) then
                ...
                pgen = excit_gen_data%pattempt_double
                pgen = pgen*calc_pgen_double_periodic(sys, ij_sym, connection%to_orb(1), &
    ...
    subroutine gen_excit_periodic_complex_no_renorm(rng, sys, excit_gen_data, cdet, pgen, connection, hmatel, allowed_excitation)
        ...
        if (get_rand_close_open(rng) < excit_gen_data%pattempt_single) then
            ...
            ! 2a. Select orbital to excite from and orbital to excite into.
            call find_ia_periodic(rng, sys, int(sys%read_in%mom_sym%gamma_sym), cdet%f, cdet%occ_list, connection%from_orb(1), &
                             connection%to_orb(1), allowed_excitation)
            connection%nexcit = 1

            if (allowed_excitation) then
                ! 3a. Probability of generating this excitation.
                pgen = excit_gen_data%pattempt_single*calc_pgen_single_periodic_no_renorm(sys, connection%to_orb(1))
            ...
            ! 2b. Select orbitals to excite from and orbitals to excite into.
            call choose_ij_periodic(rng, sys, cdet%occ_list, connection%from_orb(1), connection%from_orb(2), ij_sym, ij_spin)
            call find_ab_periodic(rng, cdet%f, ij_sym, ij_spin, sys%basis, sys%read_in%mom_sym,  &
            ...
            if (allowed_excitation) then
                ! 3b. Probability of generating this excitation.
                pgen = excit_gen_data%pattempt_double*calc_pgen_double_periodic_no_renorm(sys, connection%to_orb(1), &
    ...
    end subroutine gen_excit_periodic_complex_no_renorm
...
!--- Select random orbitals involved in a valid single excitation ---
    ...
    end subroutine choose_ia_periodic

!--- Select random orbitals involved in a valid double excitation ---

    subroutine choose_ij_periodic(rng, sys, occ_list, i, j, ij_sym, ij_spin)
        ...
        ij_sym = cross_product_periodic_basis(sys%read_in%mom_sym, i,j,sys%basis%basis_fns)
        ! ij_spin = -2 (down, down), 0 (up, down or down, up), +2 (up, up)
        ij_spin = sys%basis%basis_fns(i)%Ms + sys%basis%basis_fns(j)%Ms

    end subroutine choose_ij_periodic

    subroutine choose_ab_periodic(rng, sys, f, sym, spin, symunocc, a, b, allowed_excitation)
        ...
        if (allowed_excitation) then
            ...
            do
                ...
                if (.not.btest(f(sys%basis%bit_lookup(2,a)), sys%basis%bit_lookup(1,a))) then
                    ...
                    isymb = cross_product_periodic_read_in(sys%read_in%mom_sym, sym, &
                        sys%read_in%mom_sym%inv_sym(sys%basis%basis_fns(a)%sym))
                    ...
                    if ( (symunocc(imsb,isymb) > 1) .or. &
                        ...
                        do
                            ind = int(sys%read_in%mom_sym%nbands*get_rand_close_open(rng)) + 1
                            b = sys%read_in%mom_sym%sym_spin_basis(ind, imsb, isymb)
    ...
    end subroutine choose_ab_periodic
...
!--- Select random orbitals in single excitations ---
    ...
    subroutine find_ia_periodic(rng, sys, op_sym, f, occ_list, i, a, allowed_excitation)
        ...
        isym = cross_product_periodic_read_in(sys%read_in%mom_sym, sys%basis%basis_fns(i)%sym,op_sym)
        ind = int(sys%read_in%mom_sym%nbands*get_rand_close_open(rng))+1
        ! If using periodic system always have basis functions of given symmetry
    ...
    end subroutine find_ia_periodic

!--- Select random orbitals in double excitations ---

    subroutine find_ab_periodic(rng, f, sym, spin, basis, mom_sym, a, b, allowed_excitation)
        ...
        isym = cross_product_periodic_read_in(mom_sym, sym, mom_sym%inv_sym(basis%basis_fns(a)%sym))

        if (spin /= 0 .and. isym == basis%basis_fns(a)%sym .and. mom_sym%nbands == 1) then
            allowed_excitation = .false.
        else
            do
                ind = int(mom_sym%nbands*get_rand_close_open(rng))+1
                b = mom_sym%sym_spin_basis(ind,ims,isym)
                if (b /= a) exit
            end do
    ...
    end subroutine find_ab_periodic

!--- Excitation generation probabilities ---

    pure function calc_pgen_single_periodic(sys, op_sym, occ_list, symunocc, a) result(pgen)
    ...
    end function calc_pgen_single_periodic

    pure function calc_pgen_double_periodic(sys, ij_sym, a, b, spin, symunocc) result(pgen)
        ...
        case(-2)
            ...
            do isyma = sys%sym0, sys%sym_max
                ...
                isymb = cross_product_periodic_read_in(sys%read_in%mom_sym, sys%read_in%mom_sym%inv_sym(isyma), ij_sym)
    ...
    end function calc_pgen_double_periodic

    pure function calc_pgen_single_periodic_no_renorm(sys, a) result(pgen)
    ...
    pure function calc_pgen_double_periodic_no_renorm(sys, a, b, spin) result(pgen)
        ...
        if (isyma == isymb .and. imsa == imsb) then
            ...
            p_aijb = 1.0_p/(sys%read_in%mom_sym%nbands-1)
            p_bija = 1.0_p/(sys%read_in%mom_sym%nbands-1)
        else
            p_aijb = 1.0_p/sys%read_in%mom_sym%nbands
            p_bija = 1.0_p/sys%read_in%mom_sym%nbands
        end if

        pgen = 2.0_p/(sys%nel*(sys%nel-1)*n_aij)*(p_bija+p_aijb)

    end function calc_pgen_double_periodic_no_renorm

end module excit_gen_periodic
