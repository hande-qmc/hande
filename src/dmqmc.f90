...
contains
    ...
    subroutine do_dmqmc(sys, qmc_in, dmqmc_in, dmqmc_estimates, restart_in, load_bal_in, reference_in, qs, &
        ...
        !    sampling_probs: sampling probabilities found via the output_and_alter_weights algorithm.  Only allocated
        !       and set if dmqmc_in%find_weights is true.
        ...
        use dmqmc_data, only: dmqmc_in_t, dmqmc_estimates_t, dmqmc_weighted_sampling_t, dmqmc_in_t_json, &
                              ipdmqmc_in_t_json, rdm_in_t_json, operators_in_t_json, hartree_fock_dm
        ...
        outer_loop: do beta_cycle = 1, dmqmc_in%beta_loops
            ...
            ! DMQMC quasi-newton not functional, so we artificially set this value to 0 which should not affect non-QN
            ! calcs.
            ...
            do ireport = 1, nreport
                ...
                do icycle = 1, qmc_in%ncycles
                    ...
                    call end_mc_cycle(nspawn_events, ndeath, qs%psip_list%pop_real_factor, nattempts, &
            ...
            ! the previous loop.
            if (dmqmc_in%find_weights) call output_and_alter_weights(sys, dmqmc_in, dmqmc_estimates%excit_dist, &
                                                                    &weighted_sampling)
    ...
    subroutine init_dmqmc_beta_loop(rng, qmc_in, dmqmc_in, dmqmc_estimates, qs, beta_cycle, nstates_active, &
        ...
        if (beta_cycle /= 1 .and. parent) then
            ...
            write (iunit,'(a52,'//int_fmt(new_seed,1)//',a1)') &
                " # Resetting random number generator with a seed of:", new_seed, "."
        end if
    ...
    subroutine init_dmqmc_report_loop(calc_excit_dist, calc_mom_dist, calc_struc_fac, &
                                      bloom_stats, dmqmc_estimates, rspawn)
