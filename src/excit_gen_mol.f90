...
!--- Select random orbitals involved in a valid single excitation ---
    ...
    subroutine choose_ia_mol(rng, sys, op_sym, f, occ_list, symunocc, i, a, allowed_excitation)
        ...
        do i = 1, sys%nel
            ims = (basis_global%basis_fns(occ_list(i))%Ms+3)/2
            ! In principle here we should have (Gamma_i* Gamma_op)*.  We'll assume Gamma_op*=Gamma_op
            isym = cross_product_pg_sym(basis_global%basis_fns(occ_list(i))%sym, op_sym)
        ...
        if (allowed_excitation) then
            ...
            do
                ...
                ims = (basis_global%basis_fns(i)%Ms+3)/2
                isym = pg_sym_conj(cross_product_pg_sym(basis_global%basis_fns(i)%sym, op_sym))
                if (symunocc(ims, isym) /= 0) then
                    ! Found i.  Now find a...
                        ...
                        do
                            ...
                            if (.not.btest(f(basis_global%bit_lookup(2,a)), basis_global%bit_lookup(1,a))) exit
                        end do
                    exit
                end if
            end do
...
!--- Select random orbitals involved in a valid double excitation ---
    ...
    subroutine choose_ab_mol(rng, sys, f, sym, spin, symunocc, a, b, allowed_excitation)
        ...
        case(0)
            ...
            ! [1,basis_global%nbasis] using simply x (for compatibility with spin=-2,2).
            fac = 1
            shift = 0
            na = basis_global%nbasis
        case(2)
            do isyma = sys%sym0, sys%sym_max
                isymb = pg_sym_conj(cross_product_pg_sym(isyma, sym))
        ...
        if (allowed_excitation) then
            ...
            do
                ...
                if (.not.btest(f(basis_global%bit_lookup(2,a)), basis_global%bit_lookup(1,a))) then
                    ! b must conserve spatial and spin symmetry.
                    imsb = (spin-basis_global%basis_fns(a)%Ms+3)/2
                    isymb = pg_sym_conj(cross_product_pg_sym(sym, basis_global%basis_fns(a)%sym))
                    ! Is there a possible b?
                    if ( (symunocc(imsb,isymb) > 1) .or. &
                            (symunocc(imsb,isymb) == 1 .and. (isymb /= basis_global%basis_fns(a)%sym .or. spin == 0)) ) then
                        ! Possible b.  Find it.
                        do
                            ...
                            if ( b /= a .and. .not.btest(f(basis_global%bit_lookup(2,b)),basis_global%bit_lookup(1,b)) ) exit
                        end do
                        exit
                    end if
                end if
...
!--- Select random orbitals in single excitations ---
    ...
    subroutine find_ia_mol(rng, sys, op_sym, f, occ_list, i, a, allowed_excitation)
        ...
        ! Conserve symmetry (spatial and spin) in selecting a.
        ims = (basis_global%basis_fns(i)%Ms+3)/2
        isym = cross_product_pg_sym(basis_global%basis_fns(i)%sym,op_sym)
        ind = int(nbasis_sym_spin(ims,isym)*get_rand_close_open(rng))+1
        if (nbasis_sym_spin(ims,isym) == 0) then
            ...
            ! not permitted.
            allowed_excitation = .not.btest(f(basis_global%bit_lookup(2,a)), basis_global%bit_lookup(1,a))
        end if
...
!--- Select random orbitals in double excitations ---
    ...
    subroutine find_ab_mol(rng, f, sym, spin, a, b, allowed_excitation)
        ...
        case(-2)
            ! a must be down.
            ! Convert number, x, in range [1,nbasis/2] to even number in range
            ! [1,basis_global%nbasis] using 2*x
            fac = 2
            shift = 0
            na = basis_global%nbasis/2
        case(0)
        ...
        case(2)
            ! a must be up.
            ! Convert number, x, in range [1,nbasis/2] to odd number in range
            ! [1,basis_global%nbasis] using 2*x-1
            fac = 2
            shift = 1
            na = basis_global%nbasis/2
        end select
        ...
        ! => Ms_b = Ms_i + Ms_j - Ms_a
        ims = (spin-basis_global%basis_fns(a)%Ms+3)/2
        ! (sym_i* x sym_j* x sym_a)* = sym_b
        ! (at least for Abelian point groups)
        isym = pg_sym_conj(cross_product_pg_sym(sym, basis_global%basis_fns(a)%sym))

        if (nbasis_sym_spin(ims,isym) == 0) then
            ! No orbitals with the correct symmetry.
            allowed_excitation = .false.
        else if (spin /= 0 .and. isym == basis_global%basis_fns(a)%sym .and. nbasis_sym_spin(ims,isym) == 1) then
            allowed_excitation = .false.
        else
            do
                ind = int(nbasis_sym_spin(ims,isym)*get_rand_close_open(rng))+1
            ...
            ! not permitted.
            allowed_excitation = .not.btest(f(basis_global%bit_lookup(2,b)), basis_global%bit_lookup(1,b))
...
!--- Excitation generation probabilities ---
    ...
    pure function calc_pgen_single_mol(sys, op_sym, occ_list, symunocc, a) result(pgen)
        ...
        do i = 1, sys%nel
            ims = (basis_global%basis_fns(occ_list(i))%Ms+3)/2
            isym = pg_sym_conj(cross_product_pg_sym(basis_global%basis_fns(occ_list(i))%sym, op_sym))
            if (symunocc(ims,isym) == 0) ni = ni - 1
        end do

        ims = (basis_global%basis_fns(a)%Ms+3)/2
        isym = basis_global%basis_fns(a)%sym
    ...
    pure function calc_pgen_double_mol(sys, ij_sym, a, b, spin, symunocc) result(pgen)
        ...
        imsa = (basis_global%basis_fns(a)%ms+3)/2
        imsb = (basis_global%basis_fns(b)%ms+3)/2
        ...
        case(-2)
            ...
            ! # b.
            if (basis_global%basis_fns(a)%sym == basis_global%basis_fns(b)%sym) then
                p_aijb = 1.0_p/(symunocc(imsa,basis_global%basis_fns(a)%sym)-1)
                p_bija = 1.0_p/(symunocc(imsb,basis_global%basis_fns(b)%sym)-1)
            else
                p_aijb = 1.0_p/symunocc(imsa,basis_global%basis_fns(a)%sym)
                p_bija = 1.0_p/symunocc(imsb,basis_global%basis_fns(b)%sym)
            end if
        case(0)
            ! # a.
            n_aij = sys%nvirt
    ...
    pure function calc_pgen_double_mol_no_renorm(sys, a, b, spin) result(pgen)
        ...
        imsa = (basis_global%basis_fns(a)%ms+3)/2
        isyma = basis_global%basis_fns(a)%sym
        imsb = (basis_global%basis_fns(b)%ms+3)/2
        isymb = basis_global%basis_fns(b)%sym
