...
contains
    ...
    subroutine dmqmc_estimate_comms(dmqmc_in, error, nspawn_events, max_num_excits, ncycles, &
        ...
        call communicated_dmqmc_estimators(dmqmc_in, rep_loop_sum, min_ind, max_ind, ncycles, &
                                          &psip_list%tot_nparticles, qs, dmqmc_estimates, error)
    ...
    subroutine local_dmqmc_estimators(dmqmc_in, dmqmc_estimates, rep_loop_loc, min_ind, max_ind, nparticles, &
        ...
        if (dmqmc_in%rdm%calc_inst_rdm) then
            ...
            rep_loop_loc(min_ind(inst_rdm_trace_ind):max_ind(inst_rdm_trace_ind)) = &
                &reshape(dmqmc_estimates%inst_rdm%traces, nrdms)
        end if
    ...
    subroutine communicated_dmqmc_estimators(dmqmc_in, rep_loop_sum, min_ind, max_ind, ncycles, tot_nparticles, qs, &
        ...
        if (dmqmc_in%rdm%calc_inst_rdm) then
            dmqmc_estimates%inst_rdm%traces = &
                &real(reshape(rep_loop_sum(min_ind(inst_rdm_trace_ind):max_ind(inst_rdm_trace_ind)), &
            ...
            if (.not.sys%read_in%comp) then
                if (abs(unweighted_walker_pop(1)) > 0) then
                    ...
                    if (doing_dmqmc_calc(dmqmc_energy)) call update_dmqmc_energy_and_trace_ptr&
                        &(sys, excitation, cdet, H00, unweighted_walker_pop(1:1), psip_list%dat(1, idet), &
                        & est%trace, est%numerators(energy_ind:energy_ind))
                    ! Energy squared.
                    ...
                    if (doing_dmqmc_calc(dmqmc_HI_energy)) call update_dmqmc_HI_energy&
                        &(sys, cdet, excitation, unweighted_walker_pop(1), &
                    ...
                    if (dmqmc_in%calc_mom_dist) call update_dmqmc_momentum_distribution&
                        &(sys, cdet, excitation, H00, unweighted_walker_pop(1), est%mom_dist%f_k)
                    if (dmqmc_in%calc_struc_fac) call update_dmqmc_structure_factor_ueg&
                        &(sys, cdet, excitation, H00, unweighted_walker_pop(1), est%struc_fac%f_k)
                ...
                if (dmqmc_in%rdm%doing_rdm) then
                    call update_reduced_density_matrix_heisenberg&
                        &(sys%basis, est, dmqmc_in%rdm, cdet, excitation, psip_list%pops(:,idet), &
                        & psip_list%pop_real_factor, iteration, dmqmc_in%start_av_rdm, weighted_sampling%probs, &
                        & rdm_error)
                end if
    ...
    subroutine dmqmc_energy_squared_heisenberg(sys, cdet, excitation, H00, walker_pop, energy_sq)
        ...
        if (excitation%nexcit == 0) then
            ...
            if (sys%real_lattice%next_nearest_orbs(excitation%from_orb(1),excitation%to_orb(1)) /= 0_i0) then
                ...
                sum_H1_H2 = 4.0_p*J_coupling_squared*sys%real_lattice%&
                    &next_nearest_orbs(excitation%from_orb(1),excitation%to_orb(1))
            end if
    ...
    subroutine update_reduced_density_matrix_heisenberg(basis, dmqmc_estimates, rdm_in, cdet, excitation, walker_pop, &
                                                       &pop_real_factor, iteration, start_av_rdm, accumulated_probs,  &
                                                       &rdm_error)
    ...
    subroutine calculate_rdm_renyi_2(subsys_info, rdm_lists, accumulated_probs_old, r2)
        ...
        do irdm = 1, size(subsys_info)
            ...
            do i = 1, rdm_lists(irdm)%head(thread_id,0)
                ...
                unweighted_pop_1 = &
                    &rdm_lists(irdm)%sdata(rdm_lists(irdm)%bit_str_len+1,i)*accumulated_probs_old(excit_level)
                unweighted_pop_2 = &
                    &rdm_lists(irdm)%sdata(rdm_lists(irdm)%bit_str_len+2,i)*accumulated_probs_old(excit_level)
    ...
    subroutine update_dmqmc_HI_energy(sys, cdet, excitation, pop, tdiff, HI_energy)
        ...
        ! [TODO] Hamiltonian matrix element.
