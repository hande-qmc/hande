...
class BlockingTests1D(pdBlockTest):
    def setUp(self):
        self.data = pd.Series(base.data_1D)
    def tearDown(self):
        del self.data
    ...
    def test_weighted_pdblock(self):
        weights = pd.Series(base.weights)
        (data_len, reblock, cov) = pyblock.pd_utils.reblock(self.data,
                                                            weights=weights)
        self.check_stats(data_len, reblock, cov, base.weighted_reblock_1D)
    def test_weighted_df_pdblock(self):
        weights = pd.DataFrame({1:base.weights})
        (data_len, reblock, cov) = pyblock.pd_utils.reblock(self.data,
                                                            weights=weights)
        self.check_stats(data_len, reblock, cov, base.weighted_reblock_1D)
...
class BlockingTests2D(pdBlockTest):
    def setUp(self):
        self.data = pd.DataFrame({1:base.data_2D[0], 2:base.data_2D[1]})
    def tearDown(self):
        del self.data
    def test_pdblock(self):
        (data_len, reblock, cov) = pyblock.pd_utils.reblock(self.data)
    ...
    def test_weighted_pdblock(self):
        ...
        (data_len, reblock, cov) = pyblock.pd_utils.reblock(self.data,
                                                            weights=weights)
        self.check_stats(data_len, reblock, cov, base.weighted_reblock_2D)
    def test_2D_weighted_pdblock(self):
        weights = pd.DataFrame({1:base.weights, 2:base.weights})
