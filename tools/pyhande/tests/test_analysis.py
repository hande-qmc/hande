...
class TestProjectedEnergy(unittest.TestCase):
    ...
    def setUp(self):
        # Create dummy correlated noisy data where means gradually get
        # less noisy, standard errors find a plateau and their errors
        # stay very comparable.
        # Create dummy reblock, covariance, data_len dataframes:
        create_dummy = _CreateDummyDfs(3721)
        cols = [r'\sum H_0j N_j', 'N_0', 'alt1', 'alt2']
        means = [-100.0, 20.0, 1000.0, -1.0]
        it_optbls = [7, 6, 6, 5]
        num_reblock_its = 10
        self.reblock_orig = create_dummy.create_reblock_frame(
            cols, means, it_optbls, num_reblock_its
            )
        self.cov_orig = create_dummy.create_cov_frame(
            cols, means, num_reblock_its
            )
        self.data_len_orig = create_dummy.create_date_length_series(
            num_reblock_its
            )
    ...
    def test_basic_input(self):
        ...
        proje_dummy_optb = ['' if i != 7 else '<---' for i in range(10)]
        proje_dummy_df = pd.DataFrame.from_dict({
            'mean': proje_dummy_means,
            'standard error': proje_dummy_stes,
            'optimal block': proje_dummy_optb
            })
        proje_dummy_df = pd.concat(
            [proje_dummy_df], axis=1, keys=['Proj. Energy']
            )
        proje_dummy_df.index.name = 'reblock'
        pd.testing.assert_frame_equal(proje, proje_dummy_df, check_exact=False)
    ...
    def test_custom_cols(self):
        ...
        proje_dummy_optb = ['' if i != 6 else '<---' for i in range(10)]
        proje_dummy_df = pd.DataFrame.from_dict({
            'mean': proje_dummy_means,
            'standard error': proje_dummy_stes,
            'optimal block': proje_dummy_optb
        ...
        proje_dummy_df = pd.concat(
            [proje_dummy_df], axis=1, keys=['Proje']
            )
        proje_dummy_df.index.name = 'reblock'
        pd.testing.assert_frame_equal(proje, proje_dummy_df, check_exact=False)
...
class TestQMCSummary(unittest.TestCase):
    ...
    def setUp(self):
        ...
        create_dummy = _CreateDummyDfs(345621)
        cols = [
        ...
        # Also set up some shared dummy results.
        self.summary_dummy_0 = pd.DataFrame(
        ...
        self.summary_dummy_1 = ['Shift']

    def test_basic_input(self):
        """Test basic input."""
        summary = analysis.qmc_summary(self.reblock_orig)
        pd.testing.assert_frame_equal(
            summary[0], self.summary_dummy_0, check_exact=False
            )
        self.assertEqual(summary[1], self.summary_dummy_1)
    ...
    def test_custom_cols(self):
        ...
        summary_dummy_0_add = pd.DataFrame(
            np.asarray([[1.14092e+06, 5010.8, 404.726]]),
            columns=['mean', 'standard error', 'standard error error'],
            index=['alt1']
            )
        summary_dummy_0_add = pd.concat([
            self.summary_dummy_0, summary_dummy_0_add
            ])
        self.summary_dummy_1.append('alt2')
        pd.testing.assert_frame_equal(
            summary_cus[0], summary_dummy_0_add, check_exact=False
            )
        self.assertEqual(summary_cus[1], self.summary_dummy_1)
...
class TestExtractPopGrowth(unittest.TestCase):
    ...
    def test_basic_input_shift_immediate_variation(self):
        ...
        grow_data_dummy = pd.DataFrame(
            np.asarray([[0.968889], [-1.059212], [-0.984942], [0.99834]]).T,
            columns=self.cols
            )
        pd.testing.assert_frame_equal(
            grow_data, grow_data_dummy, check_exact=False
            )
...
class TestPlateauEstimator(unittest.TestCase):
    ...
    def test_basic_input(self):
        ...
        pd.testing.assert_frame_equal(
            shoulder, shoulder_dummy, check_exact=False
            )
...
class TestInefficiency(unittest.TestCase):
    ...
    def test_basic_input(self):
        ...
        ineff_dummy = pd.DataFrame(
            np.asarray([[106.299756, 15.034506]]),
            columns=['mean', 'standard error'],
            index=['Inefficiency']
            )
        pd.testing.assert_frame_equal(ineff, ineff_dummy, check_exact=False)
