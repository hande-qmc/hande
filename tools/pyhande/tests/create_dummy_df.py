"""Creating dummy dataframes/series for testing."""
import numpy as np
import pandas as pd


class _CreateDummyDfs():

    def __init__(self, seed):
        """Initialise random number generator."""
        self.rng = np.random.default_rng(seed)

    def create_qmc_frame(self, cols, means, sineTs, noise_facs,
                         frac_not_convergeds=0, num_mc_its=30):
        """Create correlated time series dummy qmc data table.
        ...
        means : list of floats
            ...
            when converged.
        sineTs : list of floats
            sineT in sin(counter * np.pi/sineT), modelling correlated
            time series with length of number of columns in qmc_df.
        ...
        qmc_df : :class:`pandas.DataFrame`
            Correlated time series dummy qmc data table with passed in
            cols as columns.
            Format:
        ...
        qmc_dict = {}
        for (col, mean, sineT, noise_fac, frac_not_converged) in\
                zip(cols, means, sineTs, noise_facs, frac_not_convergeds):
            if frac_not_converged > 1e-9:
                # frac_not_converged of the data plus/minus some random
                # number of data points are not converged.
                x_stdn = self.rng.standard_normal()
                ...
                qmc_dict[col] = [
                    np.sign(mean) *
                    np.exp(log_mean * (i+0.2*self.rng.standard_normal())/ncits)
                    for i in range(ncits)
                    ]
            else:
                not_converged_its = 0
                qmc_dict[col] = []
            # Create dummy correlated noisy data with a sine curve plus
            # random, Gaussian noise, shifted by a constant.
            qmc_dict[col].extend([
                mean
                + 0.1*abs(int(mean))*np.sin(i*np.pi/sineT)
                + noise_fac*self.rng.standard_normal()
                for i in range(num_mc_its - not_converged_its)
                ])
        return pd.DataFrame.from_dict(qmc_dict)

    def create_reblock_frame(self, cols, means, it_optbls, num_reblock_its=10):
        """Create dummy covariance dataframe.
        ...
        for (col, mean, it_optbl) in zip(cols, means, it_optbls):
            reb_means = [
                mean + (n_reb_its+1-i)*0.01*mean*self.rng.standard_normal()
                for i in range(n_reb_its)
                ]
            reb_stes = (
                [abs((i+1)*0.005*mean + 0.001*mean*self.rng.standard_normal())
                 for i in range(it_optbl-1)]
                + [abs((it_optbl+1)*0.005*mean
                       + 0.001*mean*self.rng.standard_normal())
                   for _ in range(min(n_reb_its-it_optbl+1, n_reb_its))]
                )
            # where the min is there for the case optbl=0
            if col != 'Proj. Energy':
                reb_stees = [
                    abs(0.1*reb_ste*(1.0 + self.rng.standard_normal()))
    ...
    def create_cov_frame(self, cols, means, num_reblock_its=10,
                         index_name='reblock'):
        """Create dummy covariance dataframe.
        ...
        for i in range(num_reblock_its):
            ...
            cov = self.rng.uniform(size=(len(cols), len(cols)))
    ...
    def create_date_length_series(self, num_reblock_its=10):
        """Create dummy data_length dataseries.
