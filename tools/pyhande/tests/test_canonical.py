...
class TestAnalyseHFObservables(unittest.TestCase):
    ...
    def setUp(self):
        # Create dummy series containing means of various DMQMC
        ...
        cov_orig = create_dummy.create_cov_frame(self.cols, means, 1)
        # reblock iterations are not a concept here:
        self.cov_orig = cov_orig.loc[0]
        # Shared dummy results
        self.result_dummy = pd.Series(
...
class TestEstimates(unittest.TestCase):
    ...
    def setUp(self):
        # Create dummy DMQMC data as well as metadata.
        create_dummy = _CreateDummyDfs(375)
        ...
        sineTs = [3.2, 4.2, 7.1, 2.22, 5.73, 1.67]
    ...
    def test_not_ueg(self):
        ...
        ind_dummy_full = []
        for i in ind_dummy:
            ind_dummy_full.extend([i, i+'_error'])
        ind_dummy_full = ['Beta'] + ind_dummy_full
    ...
    def test_not_ueg_no_naccnatt_col(self):
        ...
        result = canonical.estimates(self.metadata1, self.data)
        ind_dummy = ['U_0', 'T_0', 'V_0', 'T_HF', 'V_HF', 'U_HF']
        ind_dummy_full = []
        for i in ind_dummy:
            ind_dummy_full.extend([i, i+'_error'])
        ind_dummy_full = ['Beta'] + ind_dummy_full
